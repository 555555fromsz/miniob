...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper, std::unique_ptr<PhysicalOperator> &oper)
{
  ...
  oper = unique_ptr<PhysicalOperator>(new UpdatePhysicalOperator(update_oper.table(),update_oper.attribute_name(),update_oper.values()));
...
RC PhysicalPlanGenerator::create_plan(JoinLogicalOperator &join_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  ASSERT(expressions.size() == 1, "join_oper logical operator's children should be 1");

  unique_ptr<Expression> expression = std::move(expressions.front());
  unique_ptr<PhysicalOperator> join_physical_oper (new NestedLoopJoinPhysicalOperator(std::move(expression)));
