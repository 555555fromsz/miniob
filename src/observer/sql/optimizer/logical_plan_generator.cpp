...
RC LogicalPlanGenerator::create_plan(SelectStmt *select_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  for (int i=size-1;i>=0;i--) {// reverse
    ...
    Table * tt=tables[i];
    tt=tt;
    unique_ptr<LogicalOperator> table_get_oper(new TableGetLogicalOperator(tables[i], ReadWriteMode::READ_ONLY));
    if (table_oper == nullptr) {
      table_oper = std::move(table_get_oper);
    } else {
      RC                                  rc = RC::SUCCESS;
      std::vector<unique_ptr<Expression>> cmp_exprs;
      std::vector<FilterStmt *> aa= select_stmt->join_on();
      aa=aa;
      const std::vector<FilterUnit *>    &filter_units = select_stmt->join_on()[i]->filter_units();
...
RC LogicalPlanGenerator::create_plan(UpdateStmt *update_stmt, std::unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  string                     attribute_name=update_stmt->attribute_name();
  ...
  unique_ptr<LogicalOperator> update_oper(new UpdateLogicalOperator(table,attribute_name,values));
